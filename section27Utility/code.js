// Random is used in different places on the page
// We create the base random function one time and then call it


/*
This code defines a function randomGenerator that takes two arguments min and max, and returns a random integer between min and max, inclusive. The rand variable is generated using Math.random() which returns a random number between 0 and 1. The formula (max - min + 1) generates the range of possible values, and adding min shifts the range to start from min. Finally, Math.floor() is used to round down the result to the nearest integer.
*/
...
// Return a random number between 1-100 on click
...
// Return a random number between Start and END on click

function randStartAndEnd() {
    let startValue = parseInt(document.getElementById("randStart").value)
    let endValue = parseInt(document.getElementById("randEnd").value)

    let newRandVal = randomGenerator(startValue, endValue)
